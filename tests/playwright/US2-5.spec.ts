// tests/playwright/US2-5.spec.ts
import { test, expect } from '@playwright/test';

const BASE_URL = 'https://6-noob7-hack-frontend.vercel.app';
const ADMIN_EMAIL = 'adminforUS2-5@gmail.com';
...
test.describe('US2-5: Change payment method', () => {
  test('TC-US2-5-1: Change payment method for unpaid reservation', async ({ page }) => {
    try {
  ...
  test('TC-US2-5-2: Prevent change for completed payment', async ({ page }) => {
    try {
      await loginAsAdmin(page);
      await page.goto(`${BASE_URL}/admin`);
