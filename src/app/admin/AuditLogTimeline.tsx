        ...
        if (entry.action === "method_change" && entry.oldMethod && entry.newMethod) {
          detail = (
            <div style={{ display: "flex", gap: "6px", alignItems: "center", marginTop: "3px" }}>
              <code style={{ fontSize: "11px", background: "#f1f5f9", padding: "1px 6px", borderRadius: "4px" }}>{entry.oldMethod}</code>
              <span style={{ color: "#94a3b8", fontSize: "11px" }}>→</span>
              <code style={{ fontSize: "11px", background: "#f1f5f9", padding: "1px 6px", borderRadius: "4px" }}>{entry.newMethod}</code>
            </div>
          );
        } else if (entry.oldStatus && entry.newStatus && entry.oldStatus !== entry.newStatus) {
          detail = (
            <div style={{ display: "flex", gap: "6px", alignItems: "center", marginTop: "3px" }}>
              <code style={{ fontSize: "11px", background: "#f1f5f9", padding: "1px 6px", borderRadius: "4px" }}>{entry.oldStatus}</code>
              <span style={{ color: "#94a3b8", fontSize: "11px" }}>→</span>
              <code style={{ fontSize: "11px", background: "#f1f5f9", padding: "1px 6px", borderRadius: "4px" }}>{entry.newStatus}</code>
            </div>
          );
        }
