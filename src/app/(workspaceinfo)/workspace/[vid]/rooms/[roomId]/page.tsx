...
export default function RoomPage() {
  ...
  const toggleSlot = (id: string, status: string) => {
    ...
    setError(""); // ✅ clear error on every slot interaction
    setSelected((prev) => {
      if (prev.includes(id)) return prev.filter((s) => s !== id);
      if (prev.length >= 3) {
        setError("You can only select up to 3 slots per booking.");
        return prev;
      }
      return [...prev, id];
    });
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      // ✅ Check existing reservations — max 3 active allowed
      const checkRes = await fetch(`${BASE}/reservations/my`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const checkJson = await checkRes.json();
      const activeReservations: any[] = checkJson.data || [];

      if (activeReservations.length >= 3) {
        setError(
          "You have reached the maximum of 3 active reservations. Please cancel one before booking again."
        );
          ...
          {showPayment && reservationId ? (
            ...
            <>
                ...
                {slots.map((slot) => (
                  <div
                    ...
                    className={`${styles.slot} ${
                      selected.includes(slot.timeSlotId)
                        ? styles.slotSelected
                        : slot.status === "booked"
                        ? styles.slotBooked
              ...
              {selected.length > 0 && (
                <>
                  <div style={{
                    ...
                    <strong style={{ fontSize: "1.2rem" }}>฿{totalPrice.toLocaleString()}</strong>
                  </div>
              ...
              <button
                ...
                style={{
                  width: "100%", padding: "15px", background: "#0891b2",
                  color: "white", border: "none", borderRadius: "8px",
                  cursor: isSubmitting || selected.length === 0 ? "not-allowed" : "pointer",
                  fontSize: "1rem", fontWeight: "bold",
                  opacity: isSubmitting || selected.length === 0 ? 0.6 : 1,
